...
// --- primitive blocks -----------------------------------------------
//
// The RPY convention, the principal-axis rotation and the block assembly all
// live outside this file now: makeRpyRotation and toSymbolicVector in
// RigidTransformConstruction, makePrincipalRotation in detail/, and
// assembleTransform in the symbolic layer. One production implementation
// each, shared with the constant-transform path.
...
struct PrincipalAxisMatch { detail::PrincipalAxis axis; bool negated; };
                                        ...
                                        const ExpressionFactory& factory)
{
    if (const auto principal = matchPrincipalAxis(unitAxis))
        return detail::makePrincipalRotation(principal->axis, principal->negated,
                                             variable, factory);
...
SymbolicTransform JointTransformBuilder::build(const KinematicJoint& joint) const
{
    ...
    const SymbolicRotation originRotation = makeRpyRotation(joint.origin.rpy, factory_);
